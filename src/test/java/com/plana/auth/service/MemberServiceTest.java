...
class MemberServiceTest {
    ...
    // 🎯 진짜 객체 (실제 테스트 대상)
    // @InjectMocks: "진짜 MemberService 만들되, 의존성은 위의 가짜들로 채워줘"
    @InjectMocks
    private MemberService memberService;            // 진짜 MemberService
    ...
    void signup_ValidRequest_Success() {
        ...
        given(memberRepository.existsByEmail(validSignupRequest.getEmail())).willReturn(false);
        ...
        SignupResponseDto result = memberService.signup(validSignupRequest);
        ...
        verify(memberRepository).save(argThat(member ->
            ...
            member.getProvider() == SocialProvider.LOCAL &&
            member.getPassword().equals("encryptedPassword") &&
            member.getRole().equals("ROLE_USER") &&
            member.getEnabled().equals(true)
        ));
    }
    ...
    void signup_DuplicateEmail_ThrowsException() {
        ...
        given(memberRepository.existsByEmail(validSignupRequest.getEmail())).willReturn(true);
    ...
    void signup_PasswordMismatch_ThrowsException() {
        ...
        assertThatThrownBy(() -> memberService.signup(mismatchRequest))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void login_MemberNotFound_ThrowsException() {
        ...
        given(memberRepository.findByEmail(validLoginRequest.getEmail())).willReturn(Optional.empty());
        
        // When & Then - 이미 구현된 예외 처리 검증
        assertThatThrownBy(() -> memberService.login(validLoginRequest))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void login_WrongPassword_ThrowsException() {
        ...
        given(memberRepository.findByEmail(validLoginRequest.getEmail())).willReturn(Optional.of(savedMember));
        given(passwordEncoder.matches(validLoginRequest.getPassword(), savedMember.getPassword())).willReturn(false);
    ...
    void login_SocialAccountAttempt_ThrowsException() {
        ...
        given(memberRepository.findByEmail(validLoginRequest.getEmail())).willReturn(Optional.of(socialMember));
    ...
    void login_DisabledAccount_ThrowsException() {
        ...
        given(memberRepository.findByEmail(validLoginRequest.getEmail())).willReturn(Optional.of(disabledMember));
